...
def categoryCoins():
    ...
    # for index in range(1,2):
    coinsList = requests.get('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&category='+f'{category}'+'&order=market_cap_desc&per_page=250&page='+f'{index}'+'&sparkline=false').json()
    for coinindex in range(len(coinsList)):
        Convert_drop_down = {}
        Convert_drop_down['id']=coinsList[coinindex]['id']
        Convert_drop_down['name']=coinsList[coinindex]['name']
